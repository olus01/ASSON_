...
export default function ResultsPage() {
  ...
  if (!data.available) return <main className="results-shell"><p className="eyebrow">ASSON ELECTIONS</p><h1>Results are not yet available.</h1><p>{data.message ?? (data.status === 'CLOSED' ? 'Voting has ended. Official results are awaiting publication by the Electoral Committee.' : 'Results are not yet available.')}</p></main>
