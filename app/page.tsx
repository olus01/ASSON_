...
type Candidate = { id: string; name: string; department: string | null; manifesto: string | null }
...
export default function Page() {
  ...
  async function verifyVoter(event: FormEvent<HTMLFormElement>) {
    ...
    const { data: rawPositions, error: positionsError } = await supabase.from('positions').select('id, title, description, display_order, candidates(id, name, department, manifesto, display_order)').eq('is_active', true).order('display_order')
    setIsChecking(false)
