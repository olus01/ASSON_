...
export async function GET() {
  ...
  const [{ data: settings, error }, { count: eligible }, { count: votes }] = await Promise.all([
    supabase.from('election_settings').select('id, status, updated_at').eq('id', 1).maybeSingle(),
...
export async function PATCH(request: Request) {
  ...
  const { data: current } = await supabase.from('election_settings').select('status').eq('id', 1).maybeSingle()
